import 'package:flutter/material.dart';

//colors
const kYellow = Color(0xffFFCA4F);
const kTransparentYellow = Color(0x33FFCA4F);
const kGreen = Color(0xff20D0C2);
const kTransparentGreen = Color(0x4420D0C2);
const kRed = Color(0xffFF5B61);
const kLightRed = Color(0xffffd6d8);
const kTransparentRed = Color(0x44FF5B61);
const kGrey = Color(0xff989898);
const kLightGrey = Color(0xffDFE1E3);
const kVeryLightGrey = Color(0xffECF0F1);

//text styles

const kSheetTitle = TextStyle(
    fontSize: 50,
    fontFamily: 'CatamaranBold',
    height: 1.3,
    color: Colors.black);

const kExplanationTitle = TextStyle(
    fontSize: 40,
    fontFamily: 'CatamaranBold',
    height: 1.1,
    color: Colors.black);

const kExplanationSubTitle = TextStyle(
    fontSize: 18,
    fontFamily: 'MuliRegular',
    color: Colors.black45,
    height: 1.2);

const kPageSubsectionTitle =
    TextStyle(fontFamily: 'CatamaranBold', fontSize: 35);

const kTimeTextStyle =
    TextStyle(fontSize: 25, fontFamily: 'OpenSansBold', color: kRed);
